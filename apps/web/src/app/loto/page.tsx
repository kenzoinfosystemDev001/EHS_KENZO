...
export default function LotoPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await apiClient<any[]>("/api/v1/loto");
      if (res.success) {
        setData(res.data || []);
      } else {
    ...
    <AppShell>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {["Active Isolations", "Pending Release"].map((kpi, idx) => (
            <div
              key={idx}
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
                <div className="space-y-4">
                  {["equipmentTag", "isolationPoint", "lockBoxNumber"].map(
                    (field) => (
                      <div key={field}>
