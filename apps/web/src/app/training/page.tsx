...
export default function TrainingPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await apiClient<any[]>("/api/v1/training");
      if (res.success) {
        setData(res.data || []);
      } else {
    ...
    <AppShell>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-900">Training Matrix</h1>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {["Compliant", "Expiring Soon", "Expired"].map((kpi, idx) => (
            <div
              key={idx}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600">
                  <tr>
                    {["employeeName", "course", "status", "expiryDate"].map(
                      (col) => (
                        <th
                          key={col}
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
                <div className="space-y-4">
                  {["course", "employeeName", "date"].map((field) => (
                    <div key={field}>
