    ...
    it('POST /api/v1/hira should create a new HIRA study in DRAFT status', async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          description: 'HIRA study for overhaul, purge, and internal catalyst inspection in reactor R-201',
          plantId,
          departmentId,
          leaderId: hseUserId,
          teamMembers: [
    ...
    it('POST /api/v1/hira/:id/activities should add an operational activity', async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          description: 'Evacuation of spent catalyst under inert nitrogen atmosphere',
          isRoutine: false,
          sequenceOrder: 1,
        })
        .expect(201);
      ...
      expect(res.body.success).toBe(true);
      expect(res.body.data.id).toBeDefined();
      expect(res.body.data.activityName).toBe('Catalyst Bed Vacuuming and Nitrogen Purging');
    ...
    it('POST /api/v1/hira/:id/activities/:activityId/hazards should compute initial and residual risk server-side', async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          hazardDescription: 'Pyrophoric catalyst dust ignition and toxic gas exposure during vacuum extraction',
          consequence: 'Flash fire inside reactor, respiratory trauma, potential personnel burns',
          ...
          controls: [
            {
              ...
              description: 'Nitrogen blanketing system with redundant continuous O2/LEL gas detectors',
              effectivenessPercent: 85,
              isExisting: true,
            },
            {
              type: ControlHierarchyType.ADMINISTRATIVE,
              description: 'Confined space entry permit, spark-proof non-conductive tooling, fire watch stationed',
              effectivenessPercent: 75,
              isExisting: true,
            },
            {
              type: ControlHierarchyType.PPE,
              description: 'Positive-pressure supplied air breathing apparatus (SCBA) and flash-fire resistant suit',
              effectivenessPercent: 80,
              isExisting: true,
            },
          ],
          alarpJustification: 'Residual risk is reduced to As Low As Reasonably Practicable via triple barrier hierarchy controls',
        })
        .expect(201);
  ...
  describe('5. Workflow Engine: Explicit Action State Transitions', () => {
    it('POST /api/v1/hira/:id/actions/submit should transition DRAFT to TEAM_REVIEW', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${hseToken}`)
        .send({
          comments: 'Hazard evaluation and hierarchy of controls completed. Submitting for safety team review.',
        })
        .expect(201);
    ...
    it('POST /api/v1/hira/:id/actions/review?decision=RECOMMEND should transition to APPROVAL_PENDING', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${hseToken}`)
        .send({
          comments: 'Team review conducted. All high-risk hazards adequately mitigated via engineering and administrative barriers.',
        })
        .expect(201);
    ...
    it('POST /api/v1/hira/:id/actions/approve should formally approve the study', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${hseToken}`)
        .send({
          comments: 'Formal management approval granted for catalyst overhaul under nitrogen purge protocols.',
        })
        .expect(201);
