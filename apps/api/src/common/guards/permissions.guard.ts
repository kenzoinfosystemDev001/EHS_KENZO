...
const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  DEPARTMENT_HEAD: [
    ...
    "INCIDENT.READ", "INCIDENT.CREATE", "INCIDENT.UPDATE", "INCIDENT.INVESTIGATE", "INCIDENT.CLASSIFY", "INCIDENT.CLOSE", "INCIDENT.APPROVE",
    "HIRA.READ", "HIRA.CREATE", "HIRA.REVIEW", "HIRA.APPROVE",
  ...
  CORPORATE_HSE: [
    "OBSERVATION.READ", "OBSERVATION.CREATE", "OBSERVATION.REVIEW", "OBSERVATION.CLOSE",
    "INCIDENT.READ", "INCIDENT.CREATE", "INCIDENT.UPDATE", "INCIDENT.INVESTIGATE", "INCIDENT.CLASSIFY", "INCIDENT.CLOSE", "INCIDENT.APPROVE",
    "HIRA.READ", "HIRA.CREATE", "HIRA.UPDATE", "HIRA.REVIEW", "HIRA.APPROVE", "HIRA.ACTIVATE",
    "PTW.READ", "PTW.CREATE", "PTW.SUBMIT", "PTW.APPROVE", "PTW.CLOSE",
    "INSPECTION.READ", "INSPECTION.CREATE", "INSPECTION.EXECUTE", "INSPECTION.SUBMIT", "INSPECTION.REVIEW",
    "AUDIT.READ", "AUDIT.PERFORM",
    "COMPLIANCE.READ", "COMPLIANCE.MANAGE",
...
function getEffectivePermissions(user: AuthenticatedUserContext): Set<string> {
  ...
  // System Admins and Enterprise Admins possess all permissions across the platform
  if (user.roles?.some((r) => r === "ADMIN" || r === "SYSTEM_ADMIN")) {
