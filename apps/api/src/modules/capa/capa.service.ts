...
export class CapaService {
  private readonly TRANSITIONS: Record<string, Record<string, string>> = {
    ...
    [CapaStatus.IN_PROGRESS]: { SUBMIT_VERIFICATION: CapaStatus.PENDING_VERIFICATION },
  ...
  async create(dto: CreateCapaDto, user: AuthenticatedUserContext) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const count = await tx.capaRecord.count({ where: { organizationId: user.organizationId } });
      ...
      await this.workflowService.getOrCreateInstance(
        user.organizationId, 'CapaRecord', capa.id, CapaStatus.OPEN, 'CAPA_STANDARD_V1', tx,
      );
      ...
      await this.auditService.log({
        ...
        entityId: capa.id,
        afterState: { status: capa.status, referenceNumber: capa.referenceNumber },
        tx,
      });
      ...
      await this.outboxService.emit({
        ...
        payload: { capaId: capa.id, referenceNumber: capa.referenceNumber, priority: capa.priority },
        tx,
      });
  ...
  async findById(id: string, user: AuthenticatedUserContext) {
    const capa = await this.prisma.capaRecord.findUnique({
      where: { id },
      include: {
        tasks: { include: { assignedTo: { select: { id: true, email: true, firstName: true, lastName: true } } } },
        verifications: { include: { verifiedBy: { select: { id: true, email: true, firstName: true, lastName: true } } } },
        incident: { select: { id: true, referenceNumber: true } },
    ...
    if (!capa || capa.organizationId !== user.organizationId) throw new NotFoundException(`CAPA [${id}] not found`);
    return capa;
  }

  async executeAction(id: string, action: string, user: AuthenticatedUserContext, dto: CapaActionDto) {
    const capa = await this.findById(id, user);
    const nextState = this.TRANSITIONS[capa.status]?.[action];
    if (!nextState) throw new BadRequestException(`Action '${action}' invalid from state '${capa.status}'`);

    return this.prisma.$transaction(async (tx) => {
      const updates: any = { status: nextState as CapaStatus };
      if (action === 'ASSIGN') {
        if (!dto.assignedToId) throw new BadRequestException('assignedToId is required for ASSIGN action');
      ...
      if (action === 'VERIFY') {
        ...
        await tx.capaVerification.create({
          data: {
            ...
            verdict: dto.effectivenessRating && dto.effectivenessRating >= 4 ? 'EFFECTIVE' : dto.effectivenessRating && dto.effectivenessRating >= 2 ? 'PARTIALLY_EFFECTIVE' : 'INEFFECTIVE',
            notes: dto.comments,
          },
        });
      }
      if (action === 'CLOSE') { updates.closedById = user.id; updates.closedAt = new Date(); }

      const updated = await tx.capaRecord.update({ where: { id: capa.id }, data: updates });

      await this.workflowService.executeTransition(
        { entityType: 'CapaRecord', entityId: capa.id, action, actor: user, comments: dto.comments, tx },
        this.TRANSITIONS,
      );
      ...
      await this.outboxService.emit({
        ...
        payload: { capaId: capa.id, fromStatus: capa.status, toStatus: nextState, actorId: user.id },
        tx,
      });
